...
Расчёт занимает около 10 секунд на обычном ноутбуке (ТЗ допускает до 5 минут). Интернет и API-ключи не нужны.
...
Поиск по gid, панель узла со связями, топ-20, фильтры. Ссылка `viewer.html#gid=<gid>` сразу открывает узел.

## AI-ассистент аналитика (опционально)

`app/` отвечает на вопросы на естественном языке по готовому графу: «кто собирает деньги с этих пятерых?»,
«карточка узла <gid>». LLM (OpenAI API) вызывает инструменты поиска по `out/graph.json` и отвечает со ссылками на узлы.
Проверки в коде, а не в промпте: каждый gid в ответе должен существовать и прийти из вызова инструмента.
Без API-ключа типовые вопросы отвечаются правилами. Пайплайну ассистент не нужен.

```bash
python -m uvicorn app.main:app --port 8000   # затем http://localhost:8000
```
...
Проверяют must have: 2 248 строк и все обязательные колонки, роли только из словаря, evidence с числами и не длиннее
